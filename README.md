# Desarrollo de Sistemas de Inteligencia Artificial

**Tecnicatura:** Ciencia de Datos e Inteligencia Artificial  
**Materia:** Desarrollo de Sistemas de Inteligencia Artificial  
**Profesora:** Valeria Feito  
**Año:** 2026  
**Estudiante:** Frione, Catalina

## Descripción del curso

Esta materia tiene como objetivo introducir los conceptos y herramientas fundamentales para el desarrollo de sistemas de Inteligencia Artificial, trabajando con programación, datos, herramientas colaborativas y control de versiones.

## Aspectos Más Interesantes del Curso

Durante el curso, uno de los aspectos que más me interesó fue aprender a trabajar con datos y herramientas de programación. También me resultó interesante conocer herramientas como Deepnote, Python y GitHub, ya que permiten organizar, desarrollar y compartir los trabajos de manera más práctica.

## Trabajos Prácticos

### TP01 - Introducción a DeepNote y Entornos de Trabajo Colaborativos

Introducción al uso de Deepnote como entorno de trabajo colaborativo para el desarrollo de actividades relacionadas con datos y programación.

[Ver TP01 - DeepNote](001-DeepNote/001%20-%20Deepnote%20-%20Frione%20Catalina.ipynb)

### TP02 - Introducción a la Programación en Python

Introducción a los conceptos básicos de programación utilizando Python.

[Ver TP02 - Python](002-PythonIntroduccion/002%20-%20Python%20Repaso%20-%20Frione%20Catalina.ipynb)

### TP03 - Python Librerías

Ejercicios prácticos realizados durante el curso para conocer y utilizar diferentes librerías de Python.